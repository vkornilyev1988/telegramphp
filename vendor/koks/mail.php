...
class mail
{
	...
	public function send($to,$subject,$message,$ContentType,$filename,$filedata)
	{
		...
		$headers .= 'Content-Type: multipart/mixed; charset=UTF-8; boundary="koks-messager"'.PHP_EOL;
		$headers .= 'Subject: '.$subject.''.PHP_EOL;
		$headers .= 'Date: '.date('r').''.PHP_EOL;
		$headers .= 'To: '.$to.PHP_EOL;
		$headers .= 'From: '.$this->from_name." <".$this->from.">".PHP_EOL;

		$headers .= '--koks-messager'.PHP_EOL;
		...
		if(count($error) == 0){
			...
			$this->smtp_write($socket,'EHLO kcs.kz');
			$this->smtp_read($socket);
